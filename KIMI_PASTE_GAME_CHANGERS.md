...
## Product truth (must match)
- User already has the content. They pick platforms. CreatorFlow adjusts format for each platform.
- CreatorFlow does NOT invent the user’s content from scratch.
- Copy must be plain, professional, easy to understand (not childish).
- Do not invent new features or fake that something works if the UI says “coming soon”.

## Problem
Users see short one-line blurbs only. They do not know WHEN or WHY to use each tool on their content, or HOW to apply it.

## What to do
1) Keep all existing feature IDs, API calls, and working UI logic.
2) On each feature CARD, show clearly:
   - Short what it does
   - When to use it (1 line)
   - Why it helps their post/content (1 line)
3) In each opened DETAIL panel, add a short “How to use this” block (numbered steps) above the controls, matching what the controls actually do.
4) Align wording with product truth (bring your content → format/adjust for platforms where that applies).
5) Honest status:
   - If a panel already says “coming soon” (A/B Testing, Content Series), keep that honesty and do not claim it fully works.
   - Do not promise real AI predictions if the backend is only template/heuristic — keep labels honest based on what the code does.
6) Dark theme, readable contrast (light text on dark).
